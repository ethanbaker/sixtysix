...
import type { SeatConfig } from "../hooks/useMatch";
...
export function SeatSetup({ seats, playerLabels, onSeatChange, onStart, onStartFastForward }: SeatSetupProps) {
    ...
    <div className={styles.overlay}>
      ...
      {([0, 1] as const).map((seat) => (
        <div key={seat} className={styles.scoreRow}>
          ...
          <select value={seatConfigToOption(seats[seat])} onChange={(e) => onSeatChange(seat, optionToSeatConfig(e.target.value as SeatOption))}>
