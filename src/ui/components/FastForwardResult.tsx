import type { SimulatedMatch } from "../hooks/useMatch";
...
export function FastForwardResult({ result, playerLabels, onNewMatch }: FastForwardResultProps) {
  ...
  return (
    <div className={styles.overlay}>
      <h2>{winner !== null ? `${playerLabels[winner]} wins the match!` : "Match complete"}</h2>
      <p>
        Final score: {finalSession.match.matchScore[0]} – {finalSession.match.matchScore[1]} ({handSummaries.length} hand
        {handSummaries.length === 1 ? "" : "s"})
      </p>
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          {handSummaries.map((hand) => (
            <tr key={hand.handNumber}>
              ...
              <td>{playerLabels[hand.winner]}</td>
              <td>{hand.matchPoints}</td>
              <td>
